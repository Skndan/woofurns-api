...
import com.skndan.robin.repo.product.CollectionRepo;
...
public class CollectionController {
    ...
    public Response list(
        ...
        Page<Collection> collectionList = collectionRepo.findAllByActive(true,
                PageRequest.of(pageNo, pageSize, sortSt));

        return Response.ok(collectionList).status(200).build();
    }

    @GET
    @Path("/{id}")
    public Response getByID(@PathParam("id") UUID id) {
        Optional<Collection> optional = collectionRepo.findById(id);

        if (optional.isPresent()) {
            Collection collection = optional.get();
            return Response.ok(collection).status(200).build();
        }
    ...
    @PUT
    @Path("/{id}")
    public Response update(@PathParam("id") UUID id, Collection greeting) {
        Optional<Collection> optional = collectionRepo.findById(id);

        if (optional.isPresent()) {
            Collection collection = optional.get();
            entityCopyUtils.copyProperties(collection, greeting);
            Collection updateproductCategory = collectionRepo.save(collection);
    ...
    public Response delete(@PathParam("id") UUID id) {
        ...
        Collection entity = collectionRepo.findById(id).orElseThrow(
